...
function [runningLikelihood, runningPosterior] = computeBayesianIterationPosterior(runningWinScore, runningPrior, sigmaLikelihood, normalizationByRowAndColumn)
	...
	if nargin<4 || isempty(normalizationByRowAndColumn)
	...
	if normalizationByRowAndColumn == 2, runningLikelihood = runningLikelihood./(repmat(sum(runningLikelihood,1, 'omitNaN'), N,1)+repmat(sum(runningLikelihood,2, 'omitNaN'), 1,M)-runningLikelihood); end
	%runningLikelihood = prod(runningWinScore,3);	

	runningPosterior = runningLikelihood.*runningPrior;
	if normalizationByRowAndColumn == 1
