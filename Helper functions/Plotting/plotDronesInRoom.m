function plotDronesInRoom(posUAVgt, roomDimensions, spotterCamInfo, scatterPtSize)
	...
	ax(1) = subplot(1,3,1); plotDronesInGCA(posUAVgt, roomDimensions, spotterCamInfo, scatterPtSize); view([0, 90]); title('Top view');
	ax(2) = subplot(1,3,2); plotDronesInGCA(posUAVgt, roomDimensions, spotterCamInfoCopy, scatterPtSize); view([0, 0]); title('Front view');
	ax(3) = subplot(1,3,3); plotDronesInGCA(posUAVgt, roomDimensions, spotterCamInfo, scatterPtSize); view([90, 0]); title('Side view');
...
function plotDronesInGCA(posUAVgt, roomDimensions, spotterCamInfo, scatterPtSize)
	...
	scatter3(posUAVgt(:,1,1), posUAVgt(:,1,2), posUAVgt(:,1,3), scatterPtSize, 1:size(posUAVgt,1), 'filled');
